...
export const rbacRoles = pgTable(
  ...
  (table) => ({
    ...
    tenantNameIdx: uniqueIndex('rbac_roles_tenant_name_unique').on(table.tenantId, table.name),
...
export const rbacPermissions = pgTable(
  ...
  (table) => ({
    actionResourceIdx: uniqueIndex('rbac_permissions_action_resource_unique').on(table.action, table.resource),
...
export const rbacRolePermissions = pgTable(
  ...
  (table) => ({
    ...
    permissionIdx: index('rbac_role_permissions_permission_idx').on(table.permissionId),
    grantedIdx: index('rbac_role_permissions_granted_idx').on(table.grantedBy, table.grantedAt),
  })
);
...
export const rbacUserRoles = pgTable(
  ...
  (table) => ({
    userRoleTenantIdx: uniqueIndex('rbac_user_roles_unique').on(table.userId, table.roleId, table.tenantId),
    userIdx: index('rbac_user_roles_user_idx').on(table.userId),
    userTenantIdx: index('rbac_user_roles_user_tenant_idx').on(table.userId, table.tenantId),
    ...
    activeIdx: index('rbac_user_roles_active_idx').on(table.userId, table.isActive),
  })
);
...
export const rbacPolicies = pgTable(
  ...
  (table) => ({
    tenantNameIdx: uniqueIndex('rbac_policies_tenant_name_unique').on(table.tenantId, table.name),
    ...
    evalIdx: index('rbac_policies_eval_idx').on(table.isActive, table.priority, table.type),
  })
);
