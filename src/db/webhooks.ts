...
export const webhookConfigs = pgTable('webhook_configs', {
  ...
  tenantEnabledIdx: index('webhook_configs_tenant_enabled_idx').on(table.tenantId, table.enabled),

  /** Unique constraint: only one webhook per URL per tenant */
  tenantUrlUnique: uniqueIndex('webhook_configs_tenant_url_unique').on(table.tenantId, table.url),
}));
 ...
 */
export const webhookSecretRotations = pgTable('webhook_secret_rotations', {
  ...
  webhookIdx: index('webhook_secret_rotations_webhook_idx').on(table.webhookId),
