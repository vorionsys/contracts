...
export const serviceAccounts = pgTable(
  ...
  (table) => ({
    ...
    clientIdUniqueIdx: uniqueIndex('service_accounts_client_id_unique_idx').on(table.clientId),
    ...
    tenantStatusIdx: index('service_accounts_tenant_status_idx').on(table.tenantId, table.status),

    // Index for status-based queries
    statusIdx: index('service_accounts_status_idx').on(table.status),

    // Index for finding accounts by name within tenant
    tenantNameIdx: index('service_accounts_tenant_name_idx').on(table.tenantId, table.name),

    // Index for audit: recently used accounts
    lastUsedAtIdx: index('service_accounts_last_used_at_idx').on(table.lastUsedAt),
  })
);
 ...
 */
export const serviceAccountAuditEventEnum = pgEnum('service_account_audit_event', [
  'created',
  'updated',
  'revoked',
  'suspended',
...
export const serviceAccountAuditLog = pgTable(
  ...
  (table) => ({
    ...
    serviceAccountIdIdx: index('service_account_audit_service_account_id_idx').on(
      table.serviceAccountId
    ),

    // Index for looking up audit by client ID (even after deletion)
    clientIdIdx: index('service_account_audit_client_id_idx').on(table.clientId),

    // Index for tenant-wide audit queries
    tenantIdIdx: index('service_account_audit_tenant_id_idx').on(table.tenantId),

    // Index for filtering by event type
    eventIdx: index('service_account_audit_event_idx').on(table.event),

    // Index for time-based queries
    timestampIdx: index('service_account_audit_timestamp_idx').on(table.timestamp),
...
export const serviceTokens = pgTable(
  ...
  (table) => ({
    ...
    serviceAccountIdIdx: index('service_tokens_service_account_id_idx').on(table.serviceAccountId),
...
export type ServiceAccountAuditLogRecord = typeof serviceAccountAuditLog.$inferSelect;
export type NewServiceAccountAuditLogRecord = typeof serviceAccountAuditLog.$inferInsert;
