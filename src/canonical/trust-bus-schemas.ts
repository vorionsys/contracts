...
/**
 * @fileoverview Trust Bus Zod schemas for cross-layer governance propagation.
 *
 ...
 *
 * These schemas previously lived in `trust-bus.ts`; they were moved here so
 * that module can remain a zero-dependency leaf for enum-only consumers.
 * The enums themselves are re-exported below for convenience.
 *
 ...
 * @module @vorionsys/contracts/canonical/trust-bus-schemas
 */

import { z } from 'zod';
import { signalTypeSchema } from './trust-signal.js';
import { GovernanceLayer, SignalPriority, BusSeverity, BusSignalType } from './trust-bus.js';

export { GovernanceLayer, SignalPriority, BusSeverity, BusSignalType };
...
export const busSignalTypeSchema = z.nativeEnum(BusSignalType, {
  errorMap: () => ({ message: 'Invalid bus signal type' }),
});
...
export const trustBusSignalSchema = z.object({
  ...
  severity: busSeveritySchema.default(BusSeverity.LOW),
