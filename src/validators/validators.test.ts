...
function validTrustFactorScores() {
  return {
    'CT-COMP': 0.75, 'CT-REL': 0.60, 'CT-OBS': 0.80, 'CT-TRANS': 0.50,
    'CT-ACCT': 0.70, 'CT-SAFE': 0.65, 'CT-SEC': 0.55, 'CT-PRIV': 0.60,
    'CT-ID': 0.70, 'OP-HUMAN': 0.80, 'OP-ALIGN': 0.75, 'OP-CONTEXT': 0.65,
    'OP-STEW': 0.60, 'SF-HUM': 0.70, 'SF-ADAPT': 0.55, 'SF-LEARN': 0.50,
  };
}
...
describe('Enum validators', () => {
  describe('trustBandSchema', () => {
    it('accepts valid trust bands', () => {
      expect(trustBandSchema.parse(TrustBand.T0_SANDBOX)).toBe(TrustBand.T0_SANDBOX);
      expect(trustBandSchema.parse(TrustBand.T7_AUTONOMOUS)).toBe(TrustBand.T7_AUTONOMOUS);
    });
  ...
  describe('observationTierSchema', () => {
    it('accepts valid observation tiers', () => {
      expect(observationTierSchema.parse('BLACK_BOX')).toBe(ObservationTier.BLACK_BOX);
      expect(observationTierSchema.parse('VERIFIED_BOX')).toBe(ObservationTier.VERIFIED_BOX);
    });
  ...
  describe('dataSensitivitySchema', () => {
    it('accepts all sensitivity levels', () => {
      expect(dataSensitivitySchema.parse('PUBLIC')).toBe(DataSensitivity.PUBLIC);
      expect(dataSensitivitySchema.parse('RESTRICTED')).toBe(DataSensitivity.RESTRICTED);
    });
  });

  describe('reversibilitySchema', () => {
    it('accepts all reversibility levels', () => {
      expect(reversibilitySchema.parse('REVERSIBLE')).toBe(Reversibility.REVERSIBLE);
      expect(reversibilitySchema.parse('IRREVERSIBLE')).toBe(Reversibility.IRREVERSIBLE);
    });
  });
...
describe('Intent validators', () => {
  describe('intentContextSchema', () => {
    ...
    it('rejects invalid environment', () => {
      expect(() => intentContextSchema.parse({ environment: 'invalid' })).toThrow();
    });
    ...
    it('rejects non-UUID parentIntentId', () => {
      expect(() => intentContextSchema.parse({ parentIntentId: 'not-a-uuid' })).toThrow();
    });
  });
...
describe('Trust profile validators', () => {
  describe('trustFactorScoresSchema', () => {
    ...
    it('rejects scores below 0', () => {
      expect(() => trustFactorScoresSchema.parse({ 'CT-COMP': -0.1 })).toThrow();
    });
...
describe('Decision validators', () => {
  describe('rateLimitSchema', () => {
    ...
    it('rejects empty resource', () => {
      expect(() => rateLimitSchema.parse({ resource: '', limit: 100, windowSeconds: 60 })).toThrow();
    });

    it('rejects non-positive limit', () => {
      expect(() => rateLimitSchema.parse({ resource: 'api', limit: 0, windowSeconds: 60 })).toThrow();
      expect(() => rateLimitSchema.parse({ resource: 'api', limit: -1, windowSeconds: 60 })).toThrow();
    });
  });
