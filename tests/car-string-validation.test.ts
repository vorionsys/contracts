...
describe('CAR Legacy Format', () => {
  it('CAR_LEGACY_REGEX accepts L6 and L7', () => {
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L6-T5@1.0.0'),
    ).toBe(true);
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L7-T7@1.0.0'),
    ).toBe(true);
  });

  it('CAR_LEGACY_REGEX accepts T6 and T7', () => {
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L3-T6@1.0.0'),
    ).toBe(true);
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L3-T7@1.0.0'),
    ).toBe(true);
  });

  it('CAR_LEGACY_REGEX rejects L8 and T8', () => {
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L8-T3@1.0.0'),
    ).toBe(false);
    expect(
      CAR_LEGACY_REGEX.test('a3i.vorion.test:AB-L3-T8@1.0.0'),
    ).toBe(false);
  });
});
